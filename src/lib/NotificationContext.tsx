...
export const NotificationProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const notify = (toast: Omit<Toast, 'id'>) => {
    let safeMessage = toast.message;
    if (safeMessage && typeof safeMessage === 'string' && (
      safeMessage.includes("The string did not match the expected pattern") ||
      safeMessage.includes("DOMException") ||
      safeMessage.includes("responseSchema") ||
      safeMessage.includes("schema validation") ||
      safeMessage.includes("Google GenAI") ||
      safeMessage.includes("expected pattern")
    )) {
      safeMessage = "Not enough cargo information to create a cargo draft. Please review the missing fields and try again.";
    }

    const id = Math.random().toString(36).substr(2, 9);
    setToasts(prev => [...prev.slice(-2), { ...toast, message: safeMessage, id }]); // max 3 visible
          ...
          {toasts.map((n) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                {n.message && (
                  <p className="text-[11px] text-on-surface-variant leading-relaxed font-mono whitespace-pre-wrap">
                    {n.message}
                  </p>
                )}
              </div>
