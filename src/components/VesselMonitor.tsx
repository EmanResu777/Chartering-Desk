...
import { MarketRequestsForm } from './MarketRequests/MarketRequestsForm';
...
export const VesselMonitor: React.FC<{
...
}> = ({ vesselList, cargoList, onCreate, onUpdate, onDelete, onBulkDelete, onToggleVisibility, selectedVessel, setSelectedVessel }) => {
  ...
  const [isTonnageSearchOpen, setIsTonnageSearchOpen] = useState(false);
  const [isAddOpenTonnageOpen, setIsAddOpenTonnageOpen] = useState(false);
  const [detailsTab, setDetailsTab] = useState<'overview' | 'insurance'>('overview');
  const [isExpOpen, setIsExpOpen] = useState(false);
  const [isMenuOpen, setIsMenuOpen] = useState(false);
  const [isDetailsOpen, setIsDetailsOpen] = useState(false);
  const [isEditingDetails, setIsEditingDetails] = useState(false);
  const [editFormData, setEditFormData] = useState<Partial<Vessel>>({});
  const [routeEstimates, setRouteEstimates] = useState<Record<string, any>>({});

  React.useEffect(() => {
    if (!selectedVessel || (!selectedVessel.latitude && !selectedVessel.openingLocation) || !cargoList || cargoList.length === 0) return;
    
    // Top 5 cargoes initially
    const topCargoes = cargoList
       .map(c => ({ c, prox: calculateProximity(selectedVessel, c) }))
       .filter(x => x.prox.proximityScore > 0)
       .sort((a, b) => b.prox.proximityScore - a.prox.proximityScore)
       .slice(0, 5)
       .map(x => x.c);

    let isActive = true;
    const vLat = selectedVessel.latitude ?? selectedVessel.openingLocation?.latitude;
    const vLng = selectedVessel.longitude ?? selectedVessel.openingLocation?.longitude;

    if (vLat != null && vLng != null) {
      for (const c of topCargoes) {
         if (c.loadLocation) {
            import('../lib/routingClient').then(({ getRouteEstimate }) => {
               getRouteEstimate({
                   fromCoordinates: { lat: vLat, lng: vLng },
                   toCoordinates: c.loadLocation!,
                   cargoId: c.id,
                   vesselId: selectedVessel.id,
                   speedKnots: 12
               }).then(res => {
                   if (isActive && res && !res.stale) {
                      setRouteEstimates(prev => ({ ...prev, [c.id!]: res }));
                   }
               });
            });
         }
  ...
  }, [selectedVessel?.id, selectedVessel?.latitude, selectedVessel?.longitude, selectedVessel?.openingLocation, cargoList]);
  ...
  const [aisFetching, setAisFetching] = useState<Record<string, boolean>>({});

  const handleRefreshAIS = async (e: React.MouseEvent, vessel: Vessel) => {
    e.stopPropagation();
    if (!vessel.id) return;
    try {
      setAisFetching(prev => ({ ...prev, [vessel.id]: true }));
      ...
      onUpdate(vessel.id, {
        ...
        positionReceivedAt: data.positionReceivedAt,
        positionSource: data.provider,
        locationConfidence: data.confidence,
        sanitizedPreview: data.sanitizedPreview,
        aisStale: data.stale,
      });

      if (selectedVessel?.id === vessel.id) {
         setSelectedVessel({ ...selectedVessel, ...data, positionSource: data.provider, locationConfidence: data.confidence, aisStale: data.stale });
      }

    } catch (error) {
      console.error('Failed to refresh AIS:', error);
      alert('Failed to refresh AIS position. Ensure you have network access and the vessel is tracked.');
    } finally {
      setAisFetching(prev => ({ ...prev, [vessel.id]: false }));
    }
  };
  ...
  return (
    <div className="flex-1 flex flex-row min-w-0 h-full overflow-hidden bg-surface relative">
      ...
      <div className="flex-1 overflow-y-auto min-w-0 h-full relative">
                ...
                <div className={cn(
                  ...
                  {!isViewer && (
                    <>
                      ...
                      <button 
                        onClick={() => {
                          setIsAddOpenTonnageOpen(true);
                          setIsMenuOpen(false);
                        }}
                        className="w-full px-4 py-2.5 text-left text-[9px] font-bold uppercase tracking-widest text-on-surface-variant hover:bg-primary/10 hover:text-primary transition-colors flex items-center gap-2 border-t border-outline/10"
                      >
        ...
        {viewMode === 'map' ? (
          <div className="h-[calc(100vh-16rem)] w-full border-t border-outline/30 relative z-0 mt-4 mx-2">
            <OpportunityMap items={filteredVessels} itemType="vessel" />
          </div>
        ) : (
                    ...
                    <div className="flex gap-2">
                       <button 
                         onClick={(e) => {
                           ...
                           if (window.innerWidth < 1280) setViewMode('map');
                         }}
        ...
        <div className="hidden xl:block xl:w-1/3 border-l border-outline/30 sticky top-0 h-[calc(100vh-160px)] z-0">
          <OpportunityMap items={filteredVessels} itemType="vessel" />
        </div>
      </div>
      )}
    ...
    {/* Side Details Panel */}
      <AnimatePresence>
        {isDetailsOpen && selectedVessel && (
          <motion.div
            initial={{ x: '100%', opacity: 0 }}
            animate={{ x: 0, opacity: 1 }}
            exit={{ x: '100%', opacity: 0 }}
            transition={{ type: 'spring', damping: 30, stiffness: 250 }}
            className="absolute top-0 right-0 w-full md:w-[480px] h-full bg-surface-container/95 backdrop-blur-3xl border-l border-outline/30 z-[100] flex flex-col shadow-[-20px_0_40px_rgba(0,0,0,0.5)]"
          >
            <div className="px-8 py-6 border-b border-outline/20 flex items-center justify-between bg-transparent">
              <div className="flex items-center gap-4">
                ...
                <h2 className="font-display text-[11px] text-on-surface font-medium uppercase tracking-[0.3em]">
                  {isEditingDetails ? 'EDIT VESSEL' : t('vessel_specs')}
                </h2>
              </div>
              <div className="flex items-center gap-2">
                {!isEditingDetails && !isViewer && (
                  <button 
                    onClick={() => {
                      setEditFormData(selectedVessel);
                      setIsEditingDetails(true);
                    }}
                    className="mr-2 text-[9px] font-bold uppercase tracking-widest text-on-surface-variant hover:text-primary transition-colors border border-outline/30 rounded-sm px-3 py-1.5"
                  >
                    Edit
                  </button>
                )}
            ...
            <div className="flex-1 overflow-y-auto px-8 py-8 space-y-10">
              {isEditingDetails ? (
                <form 
                  onSubmit={(e) => {
                    ...
                    onUpdate(selectedVessel.id, editFormData);
                    setSelectedVessel({ ...selectedVessel, ...editFormData } as Vessel);
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <input 
                          type="number"
                          value={editFormData.dwt || ''} 
                          onChange={e => setEditFormData({...editFormData, dwt: parseInt(e.target.value) || 0})}
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="block text-[9px] uppercase tracking-widest text-on-surface-variant mb-1 ml-1">GRT</label>
                        <input 
                          type="number"
                          value={editFormData.grt || ''} 
                          onChange={e => setEditFormData({...editFormData, grt: parseInt(e.target.value) || 0})}
                          className="w-full bg-surface-container border border-outline/30 px-3 py-2 text-sm text-on-surface focus:border-primary/50 outline-none rounded-sm transition-all font-mono"
                        />
                      </div>
                      <div>
                        <label className="block text-[9px] uppercase tracking-widest text-on-surface-variant mb-1 ml-1">NRT</label>
                        <input 
                          type="number"
                          value={editFormData.nrt || ''} 
                          onChange={e => setEditFormData({...editFormData, nrt: parseInt(e.target.value) || 0})}
                          className="w-full bg-surface-container border border-outline/30 px-3 py-2 text-sm text-on-surface focus:border-primary/50 outline-none rounded-sm transition-all font-mono"
                        />
                      </div>
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <input 
                          ...
                          onChange={e => setEditFormData({...editFormData, builtYear: parseInt(e.target.value) || 0})}
                          className="w-full bg-surface-container border border-outline/30 px-3 py-2 text-sm text-on-surface focus:border-primary/50 outline-none rounded-sm transition-all font-mono"
                        />
                      </div>
                      <div>
                        <label className="block text-[9px] uppercase tracking-widest text-on-surface-variant mb-1 ml-1">Holds</label>
                        <input 
                          type="number"
                          value={editFormData.holds || ''} 
                          onChange={e => setEditFormData({...editFormData, holds: parseInt(e.target.value) || 0})}
                          className="w-full bg-surface-container border border-outline/30 px-3 py-2 text-sm text-on-surface focus:border-primary/50 outline-none rounded-sm transition-all"
                        />
                      </div>
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="block text-[9px] uppercase tracking-widest text-on-surface-variant mb-1 ml-1">Cubic Capacity (Grain/Bale)</label>
                        <input 
                          value={editFormData.cubicCapacity || ''} 
                          onChange={e => setEditFormData({...editFormData, cubicCapacity: e.target.value})}
                          className="w-full bg-surface-container border border-outline/30 px-3 py-2 text-sm text-on-surface focus:border-primary/50 outline-none rounded-sm transition-all"
                        />
                      </div>
                      <div>
                        <label className="block text-[9px] uppercase tracking-widest text-on-surface-variant mb-1 ml-1">Gear / Cranes</label>
                        <input 
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="block text-[9px] uppercase tracking-widest text-on-surface-variant mb-1 ml-1">Flag</label>
                        <input 
                      ...
                      <div>
                         <label className="block text-[9px] uppercase tracking-widest text-on-surface-variant mb-1 ml-1">Owner</label>
                         <input 
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <input 
                          value={editFormData.openPort || ''} 
                          onChange={e => setEditFormData({...editFormData, openPort: e.target.value})}
                          className="w-full bg-surface-container border border-outline/30 px-3 py-2 text-sm text-on-surface focus:border-primary/50 outline-none rounded-sm transition-all"
                        />
                      </div>
                      <div>
                        <label className="block text-[9px] uppercase tracking-widest text-on-surface-variant mb-1 ml-1">Open Date</label>
                        <input 
                    ...
                    <div>
                      <label className="block text-[9px] uppercase tracking-widest text-on-surface-variant mb-1 ml-1">Broker Notes</label>
                      <textarea 
                        value={editFormData.notes || ''} 
                        onChange={e => setEditFormData({...editFormData, notes: e.target.value})}
                        className="w-full bg-surface-container border border-outline/30 px-3 py-2 text-sm text-on-surface focus:border-primary/50 outline-none rounded-sm transition-all min-h-[60px]"
                      />
                    </div>
                  </div>
                  <div className="flex justify-end gap-3 mt-6 pt-6 border-t border-outline/20">
                    <button 
                      type="button"
                      onClick={() => setIsEditingDetails(false)}
                      className="px-5 py-2 text-[10px] font-bold uppercase tracking-widest text-on-surface-variant hover:text-on-surface transition-colors"
                    >
                      Cancel
                    </button>
                    <button 
                      type="submit"
                      className="px-6 py-2 bg-primary text-on-primary text-[10px] uppercase font-bold tracking-widest rounded-sm hover:bg-primary-container transition-colors shadow-md"
                    >
                      Save Changes
                    </button>
                  </div>
              ...
              {detailsTab === 'overview' ? (
                <>
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="space-y-5 bg-surface-container-low p-6 border border-outline/20 rounded-sm">
                      <div className="flex justify-between items-center">
                         ...
                         {selectedVessel.positionSource === 'system' || !selectedVessel.positionSource ? (
                           <span className="text-on-surface-variant text-[12px] font-medium font-sans tracking-wide">Not Configured</span>
                         ) : selectedVessel.aisStale ? (
                           <span className="text-warning text-[12px] font-medium font-sans tracking-wide bg-warning/10 px-2 py-0.5 rounded-sm">Stale</span>
                         ) : (
                           <span className="text-tertiary text-[12px] font-medium font-sans tracking-wide bg-tertiary/10 px-2 py-0.5 rounded-sm flex items-center gap-2">
                      ...
                      <div className="flex justify-between items-center">
                         <span className="text-on-surface-variant text-[10px] uppercase tracking-widest font-medium">Position Source</span>
                         <span className="text-on-surface text-[12px] font-medium font-mono">{selectedVessel.positionSource || 'Unknown / Parsed'}</span>
                      </div>

                      <div className="flex justify-between items-center">
                         <span className="text-on-surface-variant text-[10px] uppercase tracking-widest font-medium">Timestamp</span>
                         <span className="text-on-surface text-[12px] font-medium font-mono">{selectedVessel.positionReceivedAt ? new Date(selectedVessel.positionReceivedAt).toLocaleString() : 'N/A'}</span>
                      </div>
                      
                      <div className="flex justify-between items-center">
                         <span className="text-on-surface-variant text-[10px] uppercase tracking-widest font-medium">Confidence</span>
                         <span className="text-on-surface text-[12px] font-medium font-sans">{selectedVessel.locationConfidence || 'N/A'}</span>
                      </div>
                      
                      {(selectedVessel.latitude != null && selectedVessel.longitude != null) && (
                        <div className="flex justify-between items-start">
                           <span className="text-on-surface-variant text-[10px] uppercase tracking-widest font-medium mt-1">Coordinates</span>
                           <div className="flex flex-col items-end gap-1">
                             <span className="text-on-surface text-[12px] font-medium font-mono">{selectedVessel.latitude?.toFixed(4)}, {selectedVessel.longitude?.toFixed(4)}</span>
                             {selectedVessel.navigationStatus && <span className="text-on-surface-variant text-[10px] font-sans">{selectedVessel.navigationStatus}</span>}
                           </div>
                        </div>
                      )}
                      ...
                      <div className="pt-2">
                        <button 
                          onClick={(e) => handleRefreshAIS(e, selectedVessel)}
                          disabled={aisFetching[selectedVessel.id]}
                          className="w-full btn-outline h-9 text-xs flex items-center justify-center gap-2"
                        >
                          {aisFetching[selectedVessel.id] ? (
                            <span className="w-3 h-3 border border-t-transparent border-primary rounded-full animate-spin" />
                          ) : (
                  ...
                  {/* Nearest Cargo */}
                  {cargoList && (selectedVessel.latitude != null || selectedVessel.openingLocation) && (
                    <div className="space-y-6">
                      <h4 className="text-[9px] text-primary font-medium uppercase tracking-[0.3em] flex items-center gap-3">
                         <div className="w-8 h-[1px] bg-primary/30"></div>
                         <Radar className="w-3 h-3" />
                         Nearest Cargo Opportunities
                      </h4>
                      <div className="bg-surface-container-low border border-outline/20 p-4 space-y-3 rounded-sm">
                         {cargoList
                           .map(c => {
                              const prox = calculateProximity(selectedVessel, c, 'vessel_monitor');
                              const route = c.id ? routeEstimates[c.id] : null;
                              if (route && route.routeDistanceNm != null) {
                                  prox.distanceNm = route.routeDistanceNm;
                                  prox.explanation = `Routing: ${route.sanitizedPreview}. ${prox.explanation}`;
                                  prox.distanceConfidence = route.confidence as any;
                              }
                              return { c, prox };
                           })
                           .filter(x => x.prox.proximityScore > 0)
                           ...
                           .slice(0, 3)
                           .map(({c, prox}) => (
                              <div key={c.id} className="flex justify-between items-center bg-surface p-3 border border-outline/20">
                                 <div>
                                    <div className="text-xs font-bold text-on-surface flex items-center gap-2">
                                       {c.commodity || 'Cargo'} ({c.quantity}t)
                                       {prox.distanceNm != null && (
                                          <span className="text-[9px] font-mono text-tertiary bg-tertiary/10 px-1 rounded-sm">
                                             {prox.distanceNm.toFixed(0)} NM
                                          </span>
                                       )}
                                    </div>
                                    <div className="text-[10px] text-on-surface-variant font-mono mt-1">
                                       {prox.proximityLabel.toUpperCase()} • Load: {c.loadPort || 'Unknown'} 
                                       {routeEstimates[c.id || ''] ? ' (Route)' : ' (Straight-line)'}
                                       • Laycan: {prox.laycanCompatibility}
                                    </div>
                                 </div>
                                 <div className="text-[10px] bg-primary/10 text-primary px-2 py-1 uppercase tracking-wider rounded-sm font-bold">
                                    {prox.proximityScore} score
                                 </div>
                              </div>
                         ))}
                         {cargoList.filter(c => calculateProximity(selectedVessel, c).proximityScore > 0).length === 0 && (
                            <div className="text-xs text-on-surface-variant italic">No fitting cargoes with known proximity found.</div>
                         )}
                      </div>
                    </div>
                  )}
                  ...
                  <div className="space-y-6">
                    <CounterpartyLinker itemId={selectedVessel.id} itemType="vessel" />
                  </div>

                  {/* Voyage Estimate Section */}
                  <div className="space-y-6">
                     <VoyageEstimateSection 
                        itemId={selectedVessel.id}
                        itemType="vessel"
                        vesselId={selectedVessel.id}
                        initialAssumptions={{
                           vesselDWT: selectedVessel.dwt ? Number(selectedVessel.dwt) : 0,
                           vesselIntakeEstimate: selectedVessel.dwt ? Number(selectedVessel.dwt) * 0.95 : 0 // rough guess
                        }}
                        className="mt-4 mb-4"
                     />
                  </div>
            ...
            <div className="p-8 bg-surface-container-low border-t border-outline/20 flex flex-col gap-4">
              <div className="flex gap-4">
                <button 
                  ...
                  className="flex-1 bg-primary text-black py-4 font-bold uppercase tracking-[0.2em] hover:bg-primary/90 transition-all flex items-center justify-center gap-3 text-[10px] rounded-sm shadow-md"
                >
                ...
                <button 
                  onClick={async () => {
                     ...
                     await createDealRoom({
                        title: "Vessel: " + selectedVessel.name,
                        status: 'draft',
                        createdByUid: user?.uid || '',
                        linkedVesselId: selectedVessel.id,
                        source: 'Vessel Item'
                     });
                     alert('Deal Room Created! Navigate to Deal Rooms tab to view.');
                  }}
                  className="flex-1 border border-outline/30 text-on-surface py-4 font-bold uppercase tracking-[0.2em] hover:border-primary/50 hover:bg-surface transition-all flex items-center justify-center gap-3 text-[10px] rounded-sm"
                >
                  Open Deal Room
                </button>
              </div>
      ...
      <AnimatePresence>
        ...
        {isTonnageSearchOpen && (
          <MarketRequestsForm type="tonnage_search" onClose={() => setIsTonnageSearchOpen(false)} />
        )}
        {isAddOpenTonnageOpen && (
          <MarketRequestsForm type="available_vessel" onClose={() => setIsAddOpenTonnageOpen(false)} />
        )}
      </AnimatePresence>
    </div>
  );
