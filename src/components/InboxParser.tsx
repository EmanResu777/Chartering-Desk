...
export const InboxParser: React.FC<InboxParserProps> = ({ networkState, emails, setEmails }) => {
          ...
          <div className="flex flex-wrap items-center gap-2 md:gap-4 min-w-0 max-w-[70%] sm:max-w-[60%]">
             {!isCompact && (
               <input 
                 ...
                 onChange={(e) => { e.stopPropagation(); toggleSelectBatch(email.id); }}
                 className="mt-0.5"
               />
             )}
  ...
  const [syncStatus, setSyncStatus] = useState<'idle' | 'queued' | 'running' | 'completed' | 'failed'>('idle');
  const [syncStats, setSyncStats] = useState<any>(null);

  const handleFetchEmails = async (isBackground = false) => {
    if (syncStatus === 'queued' || syncStatus === 'running') return; // Prevent duplicate sync button spam
    try {
      ...
      setSyncStatus('failed');
      setSyncStats({ safeErrorMessage: error.message });
      setGlobalError(error.message || "Failed to fetch emails.");
    } finally {
      if (!isBackground) setLoading(false);
      setTimeout(() => setSyncStatus('idle'), 5000); // Clear after a bit
    }
  };
          ...
          <AnimatePresence>
            {syncStatus !== 'idle' && (
              <motion.div 
                ...
                exit={{ height: 0, opacity: 0 }}
                className={cn(
                  "border-b p-2 overflow-hidden flex items-center justify-between font-mono text-[10px] uppercase tracking-wider",
                   syncStatus === 'failed' ? "bg-red-500/10 border-red-500/30 text-red-400" :
                   syncStatus === 'completed' ? "bg-[#1d9bf0]/10 border-[#1d9bf0]/30 text-[#1d9bf0]" :
                   "bg-on-surface/5 border-outline text-on-surface"
                )}
              >
                <div className="flex items-center gap-3 px-2 truncate">
                  {syncStatus === 'queued' && <RotateCw className="h-3 w-3 animate-spin opacity-50" />}
                  {syncStatus === 'running' && <RotateCw className="h-3 w-3 animate-spin text-[#1d9bf0]" />}
                  {syncStatus === 'completed' && <Check className="h-3 w-3" />}
                  {syncStatus === 'failed' && <Trash2 className="h-3 w-3" />}
                  
                  <span>
                    {syncStatus === 'queued' ? "Sync queued..." :
                     syncStatus === 'running' ? "Email sync running..." :
                     syncStatus === 'completed' ? "Sync complete." :
                     syncStatus === 'failed' ? `Sync failed: ${syncStats?.safeErrorMessage || 'Unknown error'}` : ''}
                  </span>
                  
                  {syncStats && syncStatus === 'completed' && syncStats.scannedCount !== undefined && (
                    <span className="opacity-70 ml-2 hidden sm:inline">
                      &bull; Scanned {syncStats.scannedCount} 
                      {syncStats.relevantCount !== undefined ? ` · Relevant ${syncStats.relevantCount} · Skipped ${syncStats.skippedCount}` : ''}
                    </span>
                  )}
                </div>
              </motion.div>
