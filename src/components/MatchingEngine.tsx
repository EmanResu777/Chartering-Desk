              ...
              return (
                ...
                >
                  ...
                  <div className="p-4 flex-1 flex flex-col gap-4">
                    ...
                    <div className="mt-3">
                       <CounterpartyLinker itemId={match.vesselId} itemType="vessel" />
                    </div>

                    <div className="mt-3 space-y-3">
                       <VoyageEstimateSection 
                          itemId={match.vesselId} 
                          itemType="vessel" 
                          cargoId={activeCargo.id}
                          vesselId={match.vesselId}
                       />
                       <AIDealBriefCard itemId={match.vesselId} itemType="vessel" dealType="Targeted Vessel Profile" contextContext={{ cargoId: activeCargo.id, processOfferAnalysis: true }} />
                    </div>
                  ...
                  <div className="p-2 bg-surface-container grid grid-cols-2 gap-2 border-t border-outline">
                    <button 
                      onClick={async () => {
                        setProcessingVesselId(match.vesselId);
                        try {
                           const token = await auth.currentUser?.getIdToken();
                           if (!token) throw new Error("Authentication error");
                           const resp = await fetch('/api/offers/process', {
                              method: 'POST',
                              headers: {
                                'Content-Type': 'application/json',
                                'Authorization': `Bearer ${token}`
                              },
                              body: JSON.stringify({ cargoId: activeCargo.id, vesselId: match.vesselId, eta: match.eta })
                           });
                           if (!resp.ok) {
                           ...
                           try {
                             ...
                             await createDealRoom({
                               title: activeCargo.commodity + " vs " + match.vesselId,
                               status: 'active',
                               createdByUid: auth.currentUser?.uid || '',
                               linkedCargoId: activeCargo.id,
                               linkedVesselId: match.vesselId,
                               source: 'Process Offer'
                             });
                             notify({ type: 'success', title: 'Deal Room Created', message: 'A new Deal Room was initialized.' });
                           } catch (e) {
                             console.error(e);
                           }

                           onAssign && onAssign(match.vesselId, match.eta);
                           notify({ type: 'success', title: 'Offer Processed', message: 'The offer has been safely processed and recorded.' });
                        } catch (err: any) {
                           notify({
                             type: 'error',
                             title: 'Action Failed',
                          ...
                          setProcessingVesselId(null);
                        }
                      }}
                      disabled={processingVesselId === match.vesselId}
                      className="h-10 flex flex-row items-center justify-center gap-2 bg-primary text-on-primary font-bold font-sans tracking-[0.15em] uppercase hover:opacity-90 disabled:opacity-50 transition-all text-[11px]"
                    >
                      {processingVesselId === match.vesselId && <Loader2 className="w-4 h-4 animate-spin" />}
                      PROCESS OFFER
                    </button>
