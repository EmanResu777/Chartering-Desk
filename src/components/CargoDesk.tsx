...
export const CargoDesk: React.FC<{ 
  ...
  React.useEffect(() => {
    if (!selectedCargo || !selectedCargo.loadLocation || !vesselList || vesselList.length === 0) return;
    
    // Top 5 vessels initially
    const topVessels = vesselList
       .map(v => ({ v, prox: calculateProximity(v, selectedCargo) }))
    ...
    let isActive = true;
    for (const v of topVessels) {
       if (v.latitude != null && v.longitude != null) {
          import('../lib/routingClient').then(({ getRouteEstimate }) => {
             getRouteEstimate({
                 fromCoordinates: { lat: v.latitude!, lng: v.longitude! },
                 toCoordinates: selectedCargo.loadLocation!,
                 cargoId: selectedCargo.id,
                 vesselId: v.id,
                 speedKnots: 12
             }).then(res => {
                 if (isActive && res && !res.stale) {
                    setRouteEstimates(prev => ({ ...prev, [v.id!]: res }));
                 }
             });
          });
       }
  ...
  }, [selectedCargo?.id, selectedCargo?.loadLocation, vesselList]);
  const [isNewCargoModalOpen, setIsNewCargoModalOpen] = useState(false);
  const [isSearchRequestModalOpen, setIsSearchRequestModalOpen] = useState(false);
  ...
  const [editFormData, setEditFormData] = useState<Partial<Cargo>>({});
  ...
  const fetchAiInsights = async (commodity: string) => {
    ...
    try {
      ...
      if (response.status === 402) {
         ...
         setAiInsights(`Credit Limit: ${errorData.safeMessage}`);
         return;
      }

      if (!response.ok) {
        const errorData = await response.json().catch(() => ({}));
        throw new Error(errorData.safeMessage || errorData.error || "Failed to communicate with AI");
      }
              ...
              {!isViewer && (
                <>
                  <button
                    ...
                    className="px-3 h-9 sm:h-10 shrink-0 bg-surface-container border border-outline/30 text-on-surface text-[10px] uppercase font-bold tracking-widest flex items-center justify-center rounded-sm hover:bg-surface transition-all shadow-sm"
                    title={viewMode === 'list' ? "Switch to Map View" : "Switch to List View"}
                  >
                    {viewMode === 'list' ? <MapPin className="h-4 w-4 sm:mr-2" /> : <Database className="h-4 w-4 sm:mr-2" />}
                    <span className="hidden sm:inline">{viewMode === 'list' ? "Map View" : "List View"}</span>
                  </button>
                  <button
                    onClick={() => setIsSearchRequestModalOpen(true)}
                    className="px-3 h-9 sm:h-10 shrink-0 bg-tertiary text-on-primary text-[10px] uppercase font-bold tracking-widest flex items-center justify-center rounded-sm hover:bg-tertiary/90 transition-all shadow-sm"
                    title="Create Search Request"
                  >
                    <Search className="h-4 w-4 sm:mr-2" />
                    <span className="hidden sm:inline">Search Cargo</span>
                  </button>
                  <button 
        ...
        <div className="p-4 sm:p-6 space-y-2 pb-32 lg:pb-8">
          <AnimatePresence>
            ...
            {isSearchRequestModalOpen && (
              <MarketRequestsForm type="cargo_search" onClose={() => setIsSearchRequestModalOpen(false)} />
            )}
          </AnimatePresence>
          ...
          {viewMode === 'map' && (
            <div className="mx-2 mt-4">
               <OpportunityMap items={filteredCargo} itemType="cargo" />
            </div>
          )}
            ...
            <div className="px-8 py-6 border-b border-outline/20 flex items-center justify-between bg-transparent">
              <div className="flex items-center gap-4">
                ...
                <h2 className="font-display text-[11px] text-on-surface font-medium uppercase tracking-[0.3em]">
                  {isEditingDetails ? 'EDIT CARGO' : t('cargo_specs')}
                </h2>
              </div>
            ...
            <div className="flex-1 overflow-y-auto px-8 py-8 space-y-10">
              {isEditingDetails ? (
                <form 
                  onSubmit={(e) => {
                    e.preventDefault();
                    if (!editFormData.commodity) return;
                    onUpdate(selectedCargo.id, editFormData);
                    setSelectedCargo({ ...selectedCargo, ...editFormData } as Cargo);
                ...
                >
                  <div className="space-y-4">
                    <div>
                      <label className="block text-[9px] uppercase tracking-widest text-on-surface-variant mb-1 ml-1">Commodity</label>
                      <input 
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="block text-[9px] uppercase tracking-widest text-on-surface-variant mb-1 ml-1">Quantity</label>
                        <input 
                      ...
                      <div>
                         <label className="block text-[9px] uppercase tracking-widest text-on-surface-variant mb-1 ml-1">Stowage Factor</label>
                         <input 
                           value={editFormData.stowageFactor || ''} 
                           onChange={e => setEditFormData({...editFormData, stowageFactor: e.target.value})}
                           className="w-full bg-surface-container border border-outline/30 px-3 py-2 text-sm text-on-surface focus:border-primary/50 outline-none rounded-sm transition-all"
                         />
                      </div>
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="block text-[9px] uppercase tracking-widest text-on-surface-variant mb-1 ml-1">Load Port</label>
                        <input 
                          value={editFormData.loadPort || ''} 
                          onChange={e => setEditFormData({...editFormData, loadPort: e.target.value})}
                          className="w-full bg-surface-container border border-outline/30 px-3 py-2 text-sm text-on-surface focus:border-primary/50 outline-none rounded-sm transition-all"
                        />
                      </div>
                      <div>
                        <label className="block text-[9px] uppercase tracking-widest text-on-surface-variant mb-1 ml-1">Discharge Port</label>
                        <input 
                          value={editFormData.dischargePort || ''} 
                          onChange={e => setEditFormData({...editFormData, dischargePort: e.target.value})}
                          className="w-full bg-surface-container border border-outline/30 px-3 py-2 text-sm text-on-surface focus:border-primary/50 outline-none rounded-sm transition-all"
                        />
                      </div>
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="block text-[9px] uppercase tracking-widest text-on-surface-variant mb-1 ml-1">Laycan / Dates</label>
                        <input 
                          value={editFormData.laycan || ''} 
                          onChange={e => setEditFormData({...editFormData, laycan: e.target.value})}
                          className="w-full bg-surface-container border border-outline/30 px-3 py-2 text-sm text-on-surface focus:border-primary/50 outline-none rounded-sm transition-all font-mono"
                        />
                      </div>
                      <div>
                        <label className="block text-[9px] uppercase tracking-widest text-on-surface-variant mb-1 ml-1">Terms</label>
                        <input 
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="block text-[9px] uppercase tracking-widest text-on-surface-variant mb-1 ml-1">Freight Idea / Rate</label>
                        <input 
                          value={editFormData.freightIdea || editFormData.rate || editFormData.freightRate || ''} 
                          onChange={e => setEditFormData({...editFormData, freightIdea: e.target.value})}
                          className="w-full bg-surface-container border border-outline/30 px-3 py-2 text-sm text-on-surface focus:border-primary/50 outline-none rounded-sm transition-all"
                        />
                      </div>
                      <div>
                        <label className="block text-[9px] uppercase tracking-widest text-on-surface-variant mb-1 ml-1">Commission</label>
                        <input 
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="block text-[9px] uppercase tracking-widest text-on-surface-variant mb-1 ml-1">Addcom</label>
                        <input 
                          value={editFormData.addcom || ''} 
                          onChange={e => setEditFormData({...editFormData, addcom: e.target.value})}
                          className="w-full bg-surface-container border border-outline/30 px-3 py-2 text-sm text-on-surface focus:border-primary/50 outline-none rounded-sm transition-all"
                        />
                      </div>
                    </div>
                    <div>
                      <label className="block text-[9px] uppercase tracking-widest text-on-surface-variant mb-1 ml-1">Description / Notes</label>
                      <textarea 
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="block text-[9px] uppercase tracking-widest text-on-surface-variant mb-1 ml-1">Charterer / Account</label>
                        <input 
                          value={editFormData.charterer || ''} 
                          onChange={e => setEditFormData({...editFormData, charterer: e.target.value})}
                          className="w-full bg-surface-container border border-outline/30 px-3 py-2 text-sm text-on-surface focus:border-primary/50 outline-none rounded-sm transition-all"
                        />
                      </div>
                      <div>
              ...
              {vesselList && selectedCargo.loadLocation && (
                  <div className="space-y-6">
                    <h4 className="text-[9px] text-primary font-medium uppercase tracking-[0.3em] flex items-center gap-3">
                      <div className="w-8 h-[1px] bg-primary/30"></div>
                    ...
                    <div className="bg-surface-container-low border border-outline/20 p-4 space-y-3 rounded-sm">
                      {vesselList
                        .map(v => {
                           const prox = calculateProximity(v, selectedCargo, 'cargo_desk');
                           const route = v.id ? routeEstimates[v.id] : null;
                        ...
                        .slice(0, 3)
                        .map(({v, prox}) => (
                          <div key={v.id} className="flex justify-between items-center bg-surface p-3 border border-outline/20">
                             <div>
                                ...
                                <div className="text-[10px] text-on-surface-variant font-mono mt-1">
                                  {prox.proximityLabel.toUpperCase()} • AIS: {v.positionSource || 'Unknown'} 
                                  {routeEstimates[v.id || ''] ? ' (Route)' : ' (Straight-line)'}
                                  {v.aisStale && ' (Stale)'} • Laycan: {prox.laycanCompatibility}
                                </div>
                             </div>
                      ...
                      {vesselList.filter(v => calculateProximity(v, selectedCargo).proximityScore > 0).length === 0 && (
                        <div className="text-xs text-on-surface-variant italic">No open vessels with known proximity found.</div>
                      )}
                    </div>
                  </div>
              )}
              ...
              <div className="space-y-6">
                <CounterpartyLinker itemId={selectedCargo.id} itemType="cargo" />
              </div>

              {/* AI Deal Brief Section */}
              <div className="space-y-6">
                 <VoyageEstimateSection 
                    itemId={selectedCargo.id}
                    itemType="cargo"
                    cargoId={selectedCargo.id}
                    initialAssumptions={{
                       loadPort: selectedCargo.loadPort || '',
                       dischargePort: selectedCargo.dischargePort || '',
                       cargoQuantity: Number(selectedCargo.quantity) || 0
                    }}
                    className="mt-4 mb-4"
                 />
                <AIDealBriefCard 
                ...
                <button 
                  onClick={async () => {
                     ...
                     await createDealRoom({
                        title: "Cargo: " + selectedCargo.commodity,
                        status: 'draft',
                        createdByUid: user?.uid || '',
                        linkedCargoId: selectedCargo.id,
                        source: 'Cargo Item'
                     });
