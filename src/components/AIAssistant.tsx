  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (response.status === 402) {
         ...
         throw new Error(`Credit Limit: ${errorData.safeMessage}`);
      }
              ...
              {messages.map((msg, i) => (
                <div key={i} className={cn("flex gap-3 max-w-[85%]", msg.role === 'user' ? "ml-auto flex-row-reverse" : "")}>
                  ...
                  <div className={cn(
                    ...
                    {msg.content}
                    {msg.content.includes("Credit Limit:") && (
                      <div className="mt-4">
                        <button 
                          onClick={() => window.dispatchEvent(new Event('show-pricing'))}
                          className="px-4 py-2 bg-primary/20 text-primary border border-primary/50 text-[10px] font-bold uppercase tracking-widest hover:bg-primary hover:text-on-primary transition-all duration-300"
                        >
                           Upgrade Plan
                        </button>
                      </div>
              ...
              {summaryOutput && (
                ...
                >
                  ...
                  <div className="text-on-surface text-[13px] leading-relaxed font-sans font-light tracking-wide whitespace-pre-wrap">
                    {summaryOutput}
                    {summaryOutput.includes("Credit Limit:") && (
                      <div className="mt-4">
                        <button 
                          onClick={() => window.dispatchEvent(new Event('show-pricing'))}
                          className="px-4 py-2 bg-primary/20 text-primary border border-primary/50 text-[10px] font-bold uppercase tracking-widest hover:bg-primary hover:text-on-primary transition-all duration-300"
              ...
              <div className="space-y-6">
                ...
                {draftContent && (
                  ...
                  >
                    ...
                    <div className="text-on-surface text-[13px] leading-relaxed font-sans font-light tracking-wide whitespace-pre-wrap">
                      {draftContent}
                      {draftContent.includes("Credit Limit:") && (
                        <div className="mt-4">
                          <button 
                            onClick={() => window.dispatchEvent(new Event('show-pricing'))}
                            className="px-4 py-2 bg-primary/20 text-primary border border-primary/50 text-[10px] font-bold uppercase tracking-widest hover:bg-primary hover:text-on-primary transition-all duration-300"
